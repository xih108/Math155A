/*
 * SolarModern.cpp - Version 1.1 - January 18, 2019
 *
...
GlGeomSphere Earth(12, 12);    // A sphere with 8 slices and 8 stacks
GlGeomSphere Sun(10, 10);    // A sphere with 10 slices and 10 stacks
GlGeomSphere Sun2(10, 10);
GlGeomSphere Moon2(6, 6);
GlGeomSphere Red(6, 6);
GlGeomSphere PlanetX(10,10);
...
void mySetupGeometries() {
	...
	Red.InitializeAttribLocations(vertPos_loc);
...
void myRenderScene() {
    ...
    if (spinMode) {
		...
		DayOfYearX = DayOfYearX - ((int)(DayOfYearX / 500)) * 500;  // Wrap back to be in range [0,500)
	...
	double revolveSunAngle = (DayOfYear * 12.0/ 365.0)*PI2;
	SunPosMatrix.Mult_glRotate(revolveSunAngle, 0.0, 1.0, 0.0);
	SunPosMatrix.Mult_glTranslate(0.0, 0.0, 1.2);
    SunPosMatrix.DumpByColumns(matEntries);
	glUniformMatrix4fv(modelviewMatLocation,1, false, matEntries);
	glVertexAttrib3f(vertColor_loc, 1.0f, 1.0f, 0.0f);     // Make the sun yellow
	Sun.Render();

	LinearMapR4 Sun2PosMatrix = viewMatrix;
	double revolveSun2Angle = (DayOfYear *12.0/ 365.0)*PI2;
	Sun2PosMatrix.Mult_glRotate(revolveSun2Angle, 0.0, 1.0, 0.0);
	Sun2PosMatrix.Mult_glTranslate(0.0, 0.0, -1.2);
	Sun2PosMatrix.DumpByColumns(matEntries);
	glUniformMatrix4fv(modelviewMatLocation, 1, false, matEntries);
	glVertexAttrib3f(vertColor_loc, 1.0f, 1.0f, 0.0f);     // Make the sun yellow
	Sun2.Render();
    ...
    // EarthMatrix - specifies the size of the earth and its rotation on its axisDayofyearX
	...
	EarthPosMatrix.Mult_glTranslate(5 * sin(revolveAngle), 0.0, 5 * cos(revolveAngle)); // Revolve the earth around the sun
	...
	EarthMatrix.Mult_glRotate(-24*PI/180, 0.0, 0.0,1.0);     //Tilt 24 degree
	EarthMatrix.Mult_glRotate(earthRotationAngle, sin(24 / 180 * PI), cos(24 / 180 * PI), 0.0);    // Rotate earth around (sin(24),cos(24),0.0)
	EarthMatrix.Mult_glScale(0.7);                                  // Make radius 0.7.
	...
	LinearMapR4 Moon2Matrix = EarthPosMatrix;
	double moon2RotationAngle = -(DayOfYear*24.0 / 365.0)*PI2;
	Moon2Matrix.Mult_glRotate(moon2RotationAngle, 0.0,1.0,0.0);
	Moon2Matrix.Mult_glTranslate(0.0, 0.0, 1.5);
	Moon2Matrix.Mult_glScale(0.15);
	Moon2Matrix.DumpByColumns(matEntries);
	glUniformMatrix4fv(modelviewMatLocation, 1, false, matEntries);
	glVertexAttrib3f(vertColor_loc, 0.0f, 1.0f, 0.0f);
	Moon2.Render();

	LinearMapR4 RedMatrix = MoonMatrix;
	//RedMatrix.Mult_glRotate(moonRotationAngle, 0.0, 1.0, 0.0);
	RedMatrix.Mult_glTranslate(0.0,-sqrt(2.0) / 2, sqrt(2.0) / 2);
	RedMatrix.Mult_glScale(0.1);
	RedMatrix.DumpByColumns(matEntries);
	glUniformMatrix4fv(modelviewMatLocation, 1, false, matEntries);
	glVertexAttrib3f(vertColor_loc, 1.0f, 0.0f, 0.0f);
	Red.Render();

	LinearMapR4 PlanetXMatrix = viewMatrix;
	double planetXROtationAngle = (DayOfYearX / 500)*PI2;
	PlanetXMatrix.Mult_glRotate(planetXROtationAngle,0.0,1.0,0.0);
	PlanetXMatrix.Mult_glTranslate(0.0, 0.0, 7.0);
	PlanetXMatrix.Mult_glScale(0.6);
	PlanetXMatrix.DumpByColumns(matEntries);
	glUniformMatrix4fv(modelviewMatLocation, 1, false, matEntries);
	glVertexAttrib3f(vertColor_loc, 1.0f, 0.0f, 1.0f);
	PlanetX.Render();
